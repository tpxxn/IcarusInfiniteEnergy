...
namespace IcarusInfiniteEnergy
{
    ...
    public class The4DPocket : MonoBehaviour
    {
        ...
        public static void Init()
        {
            ...
            _harmoney = Harmony.CreateAndPatchAll(typeof(Pocket));
            ...
            if (File.Exists(translationsPath))
            {
                ...
                if (Localization._translations["Version"] < TranslationsVersion)
                {
                    ...
                    Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("IcarusInfiniteEnergy.Assets.Strings.json")
                        .CopyTo(fs);
                    fs.Close();
        ...
        public static void Main()
        {
            ...
            HotKey = new ConfigFile(
                    $"{Paths.GameRootPath}/BepInEx/config/tpxxn.plugin.Dyson.IcarusInfiniteEnergy,cfg", true)
    ...
    public class Pocket : MonoBehaviour
    {
        ...
        private void InitUI()
        {
            _4dAssetBundle = AssetBundle.LoadFromStream(Assembly.GetExecutingAssembly()
                .GetManifestResourceStream("IcarusInfiniteEnergy.Assets.4dpocket.ab"));
        ...
        private void AddFavorite(FavStorageItem favStorageItem, bool initializing = false)
        {
            ...
            fav.transform.Find("Btn_Del").GetComponent<Button>().onClick
                .AddListener(() => { OnFavoriteDelBtnClick(fav.transform, favStorageItem); });
        ...
        private void OnFavoriteDelBtnClick(Transform parent, FavStorageItem fav)
        {
                ...
                GameMain.data.factories[factoryIndex] != null)
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        ReflectionUtil.SetParentPublicProperty(_uiStation, "active", true);
                        ...
                        _uiStation.factorySystem = factory.factorySystem;
                        _uiStation.player = GameMain.mainPlayer;
                        ReflectionUtil.GetPrivateMethod(_uiStation, "OnStationIdChange").Invoke(_uiStation, null);
                        var OnNameInputSubmit = ReflectionUtil.GetPrivateMethod(_uiStation, "OnNameInputSubmit");
                        UnityAction<string> OnNameInputSubmitUnityAction = (UnityAction<string>)Delegate.CreateDelegate(typeof(UnityAction<string>), _uiStation, OnNameInputSubmit);
                        _uiStation.nameInput.onValueChanged.AddListener(OnNameInputSubmitUnityAction);
                        _uiStation.nameInput.onEndEdit.AddListener(OnNameInputSubmitUnityAction);
                        var OnPlayerIntendToTransferItems = ReflectionUtil.GetPrivateMethod(_uiStation, "OnPlayerIntendToTransferItems");
                        Action<int, int, int> OnPlayerIntendToTransferItemsAction = (Action<int, int, int>)Delegate.CreateDelegate(typeof(Action<int, int, int>), _uiStation, OnPlayerIntendToTransferItems);
                        _uiStation.player.onIntendToTransferItems += OnPlayerIntendToTransferItemsAction;
                        _uiStation.transform.SetAsLastSibling();
                        _uiGame.OpenPlayerInventory();
                        _inspectingStation = true;
                        IcarusInfiniteEnergy.logger.LogError("打开物流站成功");
                    }
                    else
                    {
                ...
                GameMain.data.factories[factoryIndex] != null)
            {
                try
                {
                    ...
                    )
                    {
                        ...
                        ReflectionUtil.SetParentPublicProperty(_uiStorage, "active", true);
                        ...
                        var OnStorageIdChange = ReflectionUtil.GetPrivateMethod(_uiStorage, "OnStorageIdChange").Invoke(_uiStorage, null);
                        ReflectionUtil.SetPrivateField(_uiStorage, "eventLock", true);
        ...
        public static bool OnPlayerInspecteeChange(UIGame __instance, EObjectType objType, int objId)
        {
            if (Instance != null)
            {
                ...
                if (Instance._inspectingStation)
                {
                    if (objType == EObjectType.Entity)
                    {
                        // Trying to inspect another object
